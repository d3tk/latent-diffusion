...
https://github.com/CompVis/ldm-transformers
-- merci
"""
...
def rank_zero_only(func):
    """Decorator to run function only on rank 0 (for compatibility)"""
...
class DDPM(nn.Module):
    ...
    def __init__(
        ...
        # Validate UNet configuration for cross-attention consistency
        if unet_config.get("params", {}).get("context_dim") is not None:
            use_spatial_transformer = unet_config.get("params", {}).get(
                "use_spatial_transformer", False
            )
            if not use_spatial_transformer:
                raise ValueError(
                    "When context_dim is provided in UNet config, use_spatial_transformer must be True. "
                    "This enables cross-attention conditioning. Set use_spatial_transformer=True or context_dim=None."
                )
    ...
    def p_sample_loop(self, shape, return_intermediates=False):
        ...
        for i in tqdm(timesteps, desc="Sampling t", total=self.num_timesteps):
            img = self.p_sample(
                img,
    ...
    def log_images(
        ...
        **kwargs,
        ...
        if sample:
            # get denoise row
            samples, denoise_row = self.p_sample_loop(
                (N, self.channels, self.image_size, self.image_size),
                return_intermediates=True,
                **kwargs,
            )
        else:
            samples = self.q_sample(
                batch,
                torch.randint(
        ...
        log = dict()
        for i in range(min(N, len(denoise_row))):
            log[f"denoise_row_{i}"] = denoise_row[i]
        log["samples"] = self._get_rows_from_list(samples)
        return log
...
class Layout2ImgDiffusion(DDPM):
    ...
    def log_images(self, batch, N=8, *args, **kwargs):
        ...
        # Only process bbox images if we have access to the dataset
        if (
            hasattr(self, "trainer")
            and hasattr(self.trainer, "datamodule")
            and hasattr(self.trainer.datamodule, "datasets")
        ):
            ...
            # Fallback: just log the raw bbox data if dataset is not available
            if self.cond_stage_key in batch:
                logs["bbox_raw"] = batch[self.cond_stage_key][:N]
...
class LatentDiffusion(DDPM):
    ...
    def p_sample_loop(
        ...
        if start_T is not None:
            timesteps = min(timesteps, start_T)
        iterator = (
            tqdm(reversed(range(0, timesteps)), desc="DDPM Sampling", total=timesteps)
            if verbose
            else reversed(range(0, timesteps))
        )
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, verbose=False, **kwargs):
        ...
        if ddim:
            ...
            samples, intermediates = ddim_sampler.sample(
                ddim_steps, batch_size, shape, cond, verbose=verbose, **kwargs
            )
