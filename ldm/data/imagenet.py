...
class ImageNetTrain(ImageNetBase):
    ...
    def _prepare(self):
        if self.data_root:
            # Check if we have the standard structure or the user's structure
            standard_root = os.path.join(self.data_root, self.NAME)
            user_train_dir = os.path.join(self.data_root, "train")

            if os.path.exists(user_train_dir):
                # Use the user's structure
                self.root = self.data_root
                self.datadir = user_train_dir
                print(f"Using existing ImageNet structure: {self.datadir}")
            else:
        ...
        # Check if we're using the user's structure
        if self.data_root and os.path.exists(os.path.join(self.data_root, "train")):
            # User's structure - create filelist if it doesn't exist
            if not os.path.exists(self.txt_filelist):
                print(
                    f"Creating filelist for existing ImageNet structure: {self.datadir}"
                )
...
class ImageNetValidation(ImageNetBase):
    ...
    def _prepare(self):
        if self.data_root:
            # Check if we have the standard structure or the user's structure
            standard_root = os.path.join(self.data_root, self.NAME)
            user_val_dir = os.path.join(self.data_root, "val")

            if os.path.exists(user_val_dir):
                # Use the user's structure
                self.root = self.data_root
                self.datadir = user_val_dir
                print(f"Using existing ImageNet validation structure: {self.datadir}")
            else:
                # Use the standard structure
                self.root = standard_root
                self.datadir = os.path.join(self.root, "data")
        ...
        # Check if we're using the user's structure
        if self.data_root and os.path.exists(os.path.join(self.data_root, "val")):
            # User's structure - create filelist if it doesn't exist
            if not os.path.exists(self.txt_filelist):
                print(
                    f"Creating filelist for existing ImageNet validation structure: {self.datadir}"
                )
                filelist = glob.glob(os.path.join(self.datadir, "**", "*.JPEG"))
                filelist = [os.path.relpath(p, start=self.datadir) for p in filelist]
                filelist = sorted(filelist)
